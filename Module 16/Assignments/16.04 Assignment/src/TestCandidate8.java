...
public class TestCandidate8 
{
	...
	//delete by location
	public static ArrayList<Candidate2> deleteByLoc(ArrayList<Candidate2> array, int position)
	{
		ArrayList<Candidate2> returnArray = array;
	...
	//delete by name
	public static ArrayList<Candidate2> deleteByName(ArrayList<Candidate2> array, String name)
	{
		...
		ArrayList<Candidate2> returnArray = new ArrayList<Candidate2>(array.size() - occurencesOfName);
		
		//initialize each to an empty candidate
		for(int i = 0; i < array.size() - occurencesOfName; i++)
		{
			returnArray.add(new Candidate2("Name", 0));
		}
		...
		for(int i = 0; i < returnArray.size(); i++)
		{
			...
			while(!foundNewValue)
			{
				if(!array.get(arrayIndex).getName().equalsIgnoreCase(name))
				{
					returnArray.set(i, array.get(arrayIndex));
					foundNewValue = true;
					arrayIndex++;
				}
				else
